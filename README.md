...
- **内存优化：剪贴板图片改用缩略图预览** — 之前每张全屏截图都按原分辨率（如 1920×1080 ≈ 8MB 解码位图）渲染进 DOM，几十张即可吃掉上百 MB。现 `ClipItem` 增加 `thumb` 字段，捕获时即生成最长边 ≤400px 的缩略图；`get_history` 返回及列表预览只下发缩略图，写回剪贴板仍用原图（清晰度不丢）。旧历史首次加载时自动回填缩略图并持久化。前端图片加 `loading="lazy"`，兜底轮询从 600ms 降到 1500ms。Tauri/WebView2(Chromium) 本身约 100–150MB 基线无法消除，但可控的图片内存大幅下降。

### v1.1.10

- **UI 紧凑化：压缩标题栏与剪贴板卡片之间的空白** — 减少 `.list-header` 高度并在无状态文本时完全折叠，收紧 `.card-grid` 与卡片头部的上内边距；上下贴边模式的卡片间距也同步收窄，让内容区域更充实。

### v1.1.9

- **改版：停靠尺寸改为屏幕占比** — 不再使用固定像素：
  - 默认尺寸：上下贴边占 **1/3 屏高**，左右贴边占 **1/4 屏宽**（另一维始终占满工作区保证贴边）。
  - 拖拽后按**占比**记忆（而非像素）：重开、换分辨率、改 DPI 缩放都保持同样的屏幕占比；拖到 <8% 视为未调整、>98% 视为接近满屏，均回退默认占比。
  - `WindowBounds` 新增 `heightRatio` / `widthRatio` 字段（serde default 兼容旧配置）；`position_window` 记录工作区尺寸，`Resized` 事件据此把像素换算为占比（事件回调内查 monitor 会死锁，故由停靠时缓存）。
